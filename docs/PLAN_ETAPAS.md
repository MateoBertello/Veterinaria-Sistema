...
| 7 — Guardería | Backend ✅ / Frontend casi completo (falta Modificar/Cancelar UI) | RN-GU1..GU7, RN-ME1..ME6, RN-CK1..CK6 | — | Backend cerrado: Registrar+cupo (RPC `crear_estadia_con_cupo` con `FOR UPDATE` anti-overbooking), Modificar/Cancelar (`modificar_estadia_con_cupo`/`cancelar_estadia`), Check-in/out (`hacer_checkin`/`hacer_checkout`: transiciones Reservada→EnCurso→Finalizada con marcas temporales y liberación de cupo implícita). Las tres con RPCs ejercitados de verdad en integración (ciclo de cupo cruzado, transiciones, aislamiento tenant×RPC y HTTP, REVOKE anon). **Frontend cerrado:** Registrar Estadía con cupo visual (commit `e939bee`), Ocupación por día + check-in/out (nuevo `GET /estadias?date=` con embed sin N+1, mapa de estado front-dibuja/backend-valida), **vista mensual** (calendario molde Turnos, `GET /estadias` extendido a rango en una query), **señalización de estadías vencidas sin acción** (Reservada/EnCurso con fecha pasada resaltadas, el sistema avisa y el humano actúa — auto check-out descartado por riesgo de overbooking), y botón "Volver a Ocupación" en el alta. **Pendiente para cerrar la etapa:** Modificar/Cancelar estadía en la UI (backend ya cerrado). |
