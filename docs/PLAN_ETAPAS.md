...
| :---- | :---- | :---- | :---- | :---- |
| 1 — Fundaciones | ✅ Cerrada | Fundacional: RLS/aislamiento de tenants + middlewares (bloque `rls.test.ts` + unit de middleware) | 2026-06-19 | Cierre registrado retroactivamente en E9-S1: DDL, envelope, middlewares y tests RLS de integración corriendo en verde contra la stack local desde entonces. |
...
| 7 — Guardería | ✅ Cerrada | RN-GU1..GU7, RN-ME1..ME6, RN-CK1..CK6 | 2026-07-06 | Backend cerrado: Registrar+cupo (RPC `crear_estadia_con_cupo` con `FOR UPDATE` anti-overbooking), Modificar/Cancelar (`modificar_estadia_con_cupo`/`cancelar_estadia`), Check-in/out (`hacer_checkin`/`hacer_checkout`: transiciones Reservada→EnCurso→Finalizada con marcas temporales y liberación de cupo implícita). Las tres con RPCs ejercitados de verdad en integración (ciclo de cupo cruzado, transiciones, aislamiento tenant×RPC y HTTP, REVOKE anon). Frontend cerrado: Registrar Estadía con cupo visual (commit `e939bee`), Ocupación por día + check-in/out (`GET /estadias?date=` con embed sin N+1, mapa de estado front-dibuja/backend-valida), vista mensual (calendario molde Turnos, `GET /estadias` extendido a rango en una query), señalización de estadías vencidas sin acción (Reservada/EnCurso con fecha pasada resaltadas, el sistema avisa y el humano actúa — auto check-out descartado por riesgo de overbooking), y **Modificar/Cancelar estadía** (commit `feat(web): modificar y cancelar estadía [Etapa 7]`): reusa el formulario de alta en modo edición pasando la estadía por `router state` (sin nuevo `GET /estadias/:id`, ya que cliente/mascota no son editables), check-in bloqueado si `EnCurso` (RN-ME1), cancelar con motivo obligatorio vía AlertDialog en Ocupación. |
| 8 — Plan de Vacunación | ✅ Cerrada | RN-PV1..PV9 | 2026-07-06 | Backend cerrado: plan de dosis (CRUD + marcar aplicada transaccional) y **avisos automáticos** (RN-PV6/PV7) reusando `NotificacionService` (`origen='vacunacion'`); idempotencia por el UNIQUE de `notificaciones`; ventana por-tenant (`diasAvisoVacuna`); endpoint manual `POST /notificaciones/vacunas/procesar` (cron diferido a E9, junto con turnos). Integración bloqueante verde: idempotencia real por UNIQUE, ventana dos tenants, aislamiento. Frontend cerrado: pestaña y timeline de vacunación en la ficha de mascota (commit `d8007ab`, 2026-07-06). |
| 9 — Hardening | ✅ Cerrada | Matriz completa en `docs/MATRIZ_RN_TESTS.md`: **167/167 RN con test (100 %)** | 2026-07-07 | Ejecutada como serie de 11 sub-sesiones (prompts en `docs/PROMPTS_ETAPA9.md`), S1 2026-07-06 → S11 2026-07-07. Cierre S11: DT-8 (squash de migraciones-fix con gate `db reset` + suites), DT-9/RN-AUD4 (purga de auditoría 12 meses vía pg_cron + test), seed de demo de los 3 módulos (+ eutanasia real de "Luna"), Sentry frontend mínimo (`VITE_SENTRY_DSN`), `docs/DEPLOY.md` (orden, secrets, Vault DT-10, checklist). Suites al cierre: typecheck + 417 unit + 149 integración + 286 web + 18 E2E, todas en verde. Deuda restante: ninguna abierta en TODO.md. |
