 ...
 * Rechaza con 403 MODULE_NOT_LICENSED si el módulo está deshabilitado o el tenant
 * está suspendido (activo=false).
 */
export function requireModule(modulo: ModuloVendible) {
  return async function (c: Context, next: Next): Promise<Response | void> {
    ...
    if (tenantError || !tenant || !tenant.activo) {
      moduleCache.set(key, { value: false, expiresAt: now + CACHE_TTL_MS });
      throw new DomainError(
        ErrorCode.MODULE_NOT_LICENSED,
        403,
        `El módulo '${modulo}' no está habilitado para este tenant`,
